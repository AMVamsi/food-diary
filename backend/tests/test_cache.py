...
def test_not_stale_at_23_hours():
    assert is_stale(datetime.now(timezone.utc) - timedelta(hours=23)) is False


def test_stale_at_25_hours():
    assert is_stale(datetime.now(timezone.utc) - timedelta(hours=25)) is True


def test_stale_at_24_hours_1_minute():
    assert is_stale(datetime.now(timezone.utc) - timedelta(hours=24, minutes=1)) is True
