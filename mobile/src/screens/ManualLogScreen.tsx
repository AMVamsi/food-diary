...
export default function ManualLogScreen() {
  ...
  const loadCatalogue = useCallback(async () => {
    ...
    setLoadError(null);
    try {
      const res = await client.get<{ ingredients: CatalogueItem[] }>('/logmeal/ingredients');
      ...
      setLoadError(mapApiError(err, 'Could not load ingredient catalogue — tap Retry'));
    } finally {
      setIsLoading(false);
    }
  }, [setIngredients]);
  ...
  const updateGrams = useCallback((id: number, value: string) => {
    setBasket((prev) => prev.map((b) => (b.ingredient.id === id ? { ...b, grams: value } : b)));
    setCalculatedKcal(null);
  }, []);
  ...
  const handleCalculate = useCallback(async () => {
    ...
    setCalcError(null);
    try {
      const res = await client.post<ComputeNutrientsResponse>('/logmeal/compute_nutrients', {
      ...
      setCalcError(mapApiError(err, 'Nutrition calculation failed — please try again'));
    } finally {
  ...
  const handleSave = useCallback(async () => {
    ...
    setSaveError(null);
    try {
      const totalGrams = basket.reduce((sum, b) => sum + (parseFloat(b.grams) || 0), 0);
      ...
      setSaveError(mapApiError(err, 'Could not save diary entry — please try again'));
    } finally {
      setIsSaving(false);
    }
          ...
          >
            ...
            {displayedIngredients.map((item) => {
              ...
              return (
                <TouchableOpacity
                  key={item.id}
                  style={[styles.ingredientRow, inBasket ? styles.ingredientRowSelected : null]}
                  onPress={() => toggleBasket(item)}
                  activeOpacity={0.7}
                >
                  <Text
                    style={[styles.ingredientName, inBasket ? styles.ingredientNameSelected : null]}
                    numberOfLines={2}
                  >
                    {item.name}
                  </Text>
            ...
            <View style={styles.basketSection}>
              ...
              {basket.length === 0 ? (
                <Text style={styles.emptyBasket}>Search and tap ingredients to add them here</Text>
              ) : (
                basket.map((item) => (
                  <BasketItemRow
              ...
              {calculatedKcal !== null ? (
                <View style={styles.kcalCard}>
                  ...
                  <Text style={styles.kcalValue}>{Math.round(calculatedKcal)} kcal</Text>
                </View>
              ) : null}
            ...
            <View style={styles.footerRow}>
              <View style={styles.footerBtn}>
                <Button
                  ...
                  disabled={basket.length === 0 || isCalculating || isSaving}
                />
              </View>
              <View style={styles.footerBtn}>
                <Button
                  ...
                  onPress={handleSave}
                  loading={isSaving}
                  disabled={calculatedKcal === null || isSaving || isCalculating}
                />
              </View>
            </View>
          </View>
