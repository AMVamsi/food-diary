...
export default function OtpScreen({ navigation, route }: Props) {
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      if (status === 429) {
        setError(
          'Email rate limit reached. Please wait a few minutes before requesting another code.'
        );
        startCooldown();
      } else if (status === 501) {
      ...
      >
        ...
        <View style={styles.row}>
          ...
          {cooldown > 0 ? (
            ...
            >
              <MaskedView
                maskElement={
                  <Text style={[styles.linkText, isResending && styles.dimmed]}>Resend code</Text>
                }
              >
                <LinearGradient
                ...
                >
                  <Text style={[styles.linkText, styles.linkHidden]}>Resend code</Text>
                </LinearGradient>
              </MaskedView>
            </TouchableOpacity>
          )}
        ...
        <View style={styles.row}>
          ...
          <TouchableOpacity
            ...
            hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}
          >
            <MaskedView maskElement={<Text style={styles.linkText}>Go back</Text>}>
              <LinearGradient
                colors={[colors.gradientStart, colors.gradientEnd]}
                start={{ x: 0, y: 0 }}
                end={{ x: 1, y: 0 }}
              >
                <Text style={[styles.linkText, styles.linkHidden]}>Go back</Text>
              </LinearGradient>
            </MaskedView>
          </TouchableOpacity>
        </View>
