...
function SwipeableRow({ children, onDelete, deleting, deleteError }: SwipeableRowProps) {
  ...
  return (
    <View style={swipeStyles.wrapper}>
      ...
      <Animated.View style={{ transform: [{ translateX }], backgroundColor: colors.surface }}>
        <TouchableOpacity activeOpacity={0.85} onPress={handleRowPress}>
          {children}
        </TouchableOpacity>
        {deleteError ? <Text style={swipeStyles.inlineError}>{deleteError}</Text> : null}
      </Animated.View>
    </View>
  );
}
...
function EntryRow({ entry, onThumbnailPress, onDelete, deleting, deleteError }: EntryRowProps) {
  ...
  return (
    <SwipeableRow onDelete={() => onDelete(entry.id)} deleting={deleting} deleteError={deleteError}>
      <View style={entryStyles.row}>
        {/* Thumbnail or placeholder — always 40×40 so rows align */}
        {hasImage ? (
          <TouchableOpacity onPress={() => onThumbnailPress(entry.image_url!)} activeOpacity={0.85}>
            <Image source={{ uri: entry.image_url! }} style={entryStyles.thumbnail} />
          </TouchableOpacity>
        ) : (
        ...
        <MaskedView
          maskElement={
            <View style={styles.kcalMask}>
              <Text style={styles.kcalText}>{Math.round(section.total_kcal)} kcal</Text>
            </View>
          }
        >
          <LinearGradient
